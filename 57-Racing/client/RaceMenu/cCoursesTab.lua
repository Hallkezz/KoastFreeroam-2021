...
function CoursesTab:__init(...) ; TabBase.__init(self , "Карты" , ...)
	...
	groupBoxCourseSelect:SetText("Выберите карту")
	...
	self.courseGroupBox:SetText("Карта не выбрана")
	...
	timesPlayed , title , self.timesPlayedLabel = RaceMenuUtility.CreateTitledLabel("Сыграно")
	timesPlayed:SetParent(cells[1])
	timesPlayed:SetDock(GwenPosition.Left)
	timesPlayed:SetToolTip("Общие количество гонок проходило на этой карте")
...
function CoursesTab:CreateRecordsTab()
	local tabButton = self.tabControl:AddPage("Рекорды")
	...
	self.recordsList:AddColumn("Время" , 65)
	self.recordsList:AddColumn("Транспорт")
...
function CoursesTab:CreateMapTab()
	local tabButton = self.tabControl:AddPage("Карта")
...
function CoursesTab:FillCourseList()
	...
	if #RaceMenu.cache.courses > 0 then
		...
		self.coursesList:AddItem("Карты не найдены")
		self.coursesList:SetDataBool("isValid" , false)
	end
end
