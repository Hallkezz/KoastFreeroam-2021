...
function Tasks:GetNeededs( args, sender )
...
function Tasks:GetPrize( args, sender )
    sender:SetMoney( sender:GetMoney() + 10000 )
    sender:SetNetworkValue( "Prize", 0 )
    print( sender:GetName() .. " get $10.000" )
    Events:Fire( "ToDiscordConsole", { text = "[Tasks] " .. sender:GetName() .. " get $10.000" } )
end

function Tasks:initVars()
	self.timer = Timer()
	self.delay = 24
end
...
function Tasks:PlayerJoin( args )
