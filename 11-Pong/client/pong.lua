...
function Pong:Render( args )
	if active then
		...
		Render:DrawText( pong_draw_start + Vector2( pong_table_width / 2 - 399, pong_table_height + 51 ), "Подсказка: Используйте мышь, чтобы переместить платформу, введите \"/pong exit\" чтобы выйти.", Color( 25, 25, 25, 150 ), 18 )
		Render:DrawText( pong_draw_start + Vector2( pong_table_width / 2 - 400, pong_table_height + 50 ), "Подсказка: Используйте мышь, чтобы переместить платформу, введите \"/pong exit\" чтобы выйти.", Color( 255, 255, 255 ), 18 )
