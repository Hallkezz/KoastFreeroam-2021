...
function Shop:LoadCategories()
	for category_id, category in ipairs( self.items ) do
		...
		if category_id == self.types["Остальное >"] then
			...
			local actionBtn = Button.Create( window )
			actionBtn:SetText( "Отправить деньги" )
			actionBtn:SetMargin( Vector2( 0, 2 ), Vector2( 0, 2 ) )
			actionBtn:SetTextSize( 14 )
			actionBtn:SetHeight( 30 )
			actionBtn:SetDock( GwenPosition.Top )
			actionBtn:Subscribe( "Press", function() Events:Fire( "SendMoney" ) self:Close() end )
			...
			self.toggleH:SetPosition( Vector2( 300, 5 ) )
			...
			self.buttonHB:SetText( "Установить точку дома здесь ( $1.500 )" )
			...
			self.buttonHB:SetSize( Vector2( 245, 20 ) )
...
function Shop:OpenShop()
	...
	if self.active then
		...
		LocalPlayer:GetValue( "Tag" ) == "AdminD" or LocalPlayer:GetValue( "Tag" ) == "Admin" or LocalPlayer:GetValue( "Tag" ) == "GlAdmin" or LocalPlayer:GetValue( "Tag" ) == "Creator" then
