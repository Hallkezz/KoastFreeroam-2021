...
function Player:SendErrorMessage( str )
	self:SendChatMessage( "[Чёрный рынок] ", Color.White, str, Color.DarkGray )
end

function Player:SendSuccessMessage( str )
	self:SendChatMessage( "[Чёрный рынок] ", Color.White, str, Color( 255, 170, 0 ) )
end
...
function Shop:PlayerFired( args, player )
	...
	if success then
        ...
        local str = string.format( "%s!", item:GetName() )
...
function Shop:BuyVehicle( player, item, tone1, tone2 )
	if item:GetRank() ~= nil then
		if player:GetValue( "Tag" ) == "Creator" or player:GetValue( "Tag" ) == "GlAdmin" or player:GetValue( "Tag" ) == "Admin"
			or player:GetValue( "Tag" ) == "AdminD" or player:GetValue( "Tag" ) == "ModerD" or player:GetValue( "Tag" ) == "VIP" then
			self:ExecuteVehicle( player, item, tone1, tone2 )
			return true, ""
		else
			return false, "У вас отсутствует VIP статус! Приобрести можно в группе в вк - vk.com/rusjc", Network:Send( player, "NoVipText" )
		end
	end
