...
function Settings:__init()
	...
	Network:Subscribe( "Clouds", self, self.Clouds )
	Network:Subscribe( "Thunder", self, self.Thunder )
end
...
function Settings:PlayerQuit( args )
    ...
    local cmd = SQL:Command( "INSERT OR REPLACE INTO players_settings (steamid, clockvisible, clockpendosformat, bestrecordsvisible, passivemodevisible, jesusmodevisible, killfeedvisible, chattipsvisible, chatbackgroundvisible, playersmarkersvisible, jobsmarkersvisible, customcrosshair, longergrapplevisible, jobsvisible) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)" )
	...
	cmd:Bind( 13, self:BooleanToNumber( args.player:GetValue( "LongerGrappleVisible" ) ) )
	cmd:Bind( 14, self:BooleanToNumber( args.player:GetValue( "JobsVisible" ) ) )
	cmd:Execute()
end

function Settings:GetDBSettings( args )
	local qry = SQL:Query( "SELECT clockvisible, clockpendosformat, bestrecordsvisible, passivemodevisible, jesusmodevisible, killfeedvisible, chattipsvisible, chatbackgroundvisible, playersmarkersvisible, jobsmarkersvisible, customcrosshair, longergrapplevisible, jobsvisible FROM players_settings WHERE steamid = ?")
...
function Settings:Clear( args, sender )
	if sender:GetWorld() ~= DefaultWorld then return end
	sender:SetWeatherSeverity( 0 )
end

function Settings:Clouds( args, sender )
	if sender:GetWorld() ~= DefaultWorld then return end
	sender:SetWeatherSeverity( 1 )
end

function Settings:Thunder( args, sender )
	if sender:GetWorld() ~= DefaultWorld then return end
