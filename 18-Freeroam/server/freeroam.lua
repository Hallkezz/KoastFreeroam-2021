...
local continuousFireTable = { [66] = true, [116] = true }

local velueses = { 30, 40, 50, 60, 70, 80, 90, 100, 150, 200, 20, 15, 25, 35, 45, 55, 65, 75, 85, 95, 120, 170 }
local veluesesTw = { 200, 250, 300, 350, 400, 450, 500 }
local veluesesTh = { 550, 600, 650, 700, 750, 800, 850, 950, 1000 }
...
function ChatHandlers:teleport( args )
    ...
    if dest == "" or dest == nil or dest == "help" then
        args.player:SendChatMessage( "Места для телепортации: ", 
                                        Color( 0, 255, 0 ) )
    ...
    elseif self.teleports[dest] ~= nil then
        if args.player:GetWorld() ~= DefaultWorld then
            args.player:SendChatMessage( 
                "Вы не в главном мире! Выйдите из любых режимов и повторите попытку.",
                Color( 255, 0, 0 ) )
        ...
        args.player:SendChatMessage( "Недопустимый пункт телепортации!", 
                                        Color( 255, 0, 0 ) )
    end
end
...
function Freeroam:PlayerJoin( args )
	...
	if #result > 0 then
        for i, v in ipairs(result) do
            if tonumber(v.possX) and tonumber(v.possY) and tonumber(v.possZ) then
        ...
        if args.killer:GetValue( "Passive" ) then
            ...
            if args.killer:GetValue( "Kills" ) then
                ...
                if args.player:GetValue( "Kills" ) == 0 then
                    ...
                    if continuousFireTable[args.killer:GetEquippedWeapon().id] == true and not args.killer:InVehicle() then
                ...
                elseif args.player:GetValue( "Kills" ) >= 0 then
                    if not continuousFireTable[args.killer:GetEquippedWeapon().id] == true and not args.killer:InVehicle() then
