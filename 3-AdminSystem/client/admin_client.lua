...
function Admin:__init()
	...
	-- Normal events
	Events:Subscribe( "CalcView", self, self.spectateCamera )
...
function Admin:LoadPanel()
	...
	self.panel.main.warpPlayerTo = GUI:Button( "Тп игрока к...", Vector2 ( 0.36, 0.32 ), Vector2 ( 0.123, 0.03 ), self.panel.main.playersTab, "player.warpto" )
	...
	self.panel.warp.window = GUI:Window( "Телепортировать игрока к...", Vector2.Zero, Vector2 ( 0.2, 0.45 ) )
	...
	self.panel.main.sendMessage = GUI:Button( ">", Vector2 ( 0.385, 0.66 ), Vector2 ( 0.05, 0.04 ), self.panel.main.adminchatTab )
	self.panel.main.sendMessage:Subscribe( "Press", self, self.sendChatMessage )
	self.panel.main.clearMessage = GUI:Button( "Очистить", Vector2 ( 0.44, 0.66 ), Vector2 ( 0.04, 0.04 ), self.panel.main.adminchatTab, "adminchat.clearmessages" )
...
function Admin:spectate()
	...
	if ( player ) then
		...
		if ( not spectateEvent ) then
			self.speccing = not self.speccing
		else
...
function Admin:spectateCamera()
	if not self.speccing then return end
	if IsValid ( self.victim, false ) then
...
function Admin:giveVehicle()
	...
	if time < self.cooltime then
		Chat:Print( "[Сервер] ", Color.White, "Подождите " .. math.ceil(self.cooltime - time) .. " секунд, чтобы выдать транспорт!", Color( 255, 34, 34 ) )
		return
	end
...
function Admin:showVehicleColourSelector()
	...
	if ( player ) then
		if player:InVehicle() then
			...
			Chat:Print( "Этот игрок находится не в автомобиле.", err )
		end
	else
		Chat:Print( NoSelected, err )
	end
...
function Admin:giveWeapon()
	local time = Client:GetElapsedSeconds()
	if time < self.cooltime then
		Chat:Print( "[Сервер] ", Color.White, "Подождите " .. math.ceil(self.cooltime - time) .. " секунд, чтобы выдать оружие!", Color( 255, 34, 34 ) )
		return
	end
...
function Admin:renderShout()
	Render:DrawText( Vector2 ( Render.Width / 2 - ( Render:GetTextWidth ( self.shoutName ) / 2 ), self.sy / 2 ) + Vector2.One, self.shoutName, Color( 0, 0, 0, 150 ), 30 )
	Render:DrawText( Vector2 ( Render.Width / 2 - ( Render:GetTextWidth ( self.shoutMessage ) / 2 ), self.sy / 2 + 50 ) + Vector2.One, self.shoutMessage, Color( 0, 0, 0, 150 ), 30 )
