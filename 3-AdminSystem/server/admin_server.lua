		...
		if ACL:hasObjectPermissionTo ( tostring ( player:GetSteamId() ), args [ 1 ] ) then
			...
			elseif ( args [ 1 ] == "player.kick" ) then
				if IsValid ( args [ 2 ], false ) then
					...
					Chat:Broadcast( "[Сервер] ", Color.White, args [ 2 ]:GetName() .." был кикнут ".. player:GetName() .." ( ".. reason .." )", Color ( 255, 0, 0 ) )
...
--					end
			...
			elseif ( args [ 1 ] == "player.warn" ) then
				if IsValid ( args [ 2 ], false ) then
					local reason = tostring ( args [ 3 ] or "No reason defined" )
					Chat:Broadcast( "[Сервер] ", Color.White, args [ 2 ]:GetName() .." получил предупреждение! ( ".. reason .." )", Color ( 255, 0, 0 ) )
				...
				if IsValid ( args [ 2 ], false ) then
					if mute:isPlayerMuted ( args [ 2 ] ) then
						if mute:setPlayerMuted ( { steamID = tostring ( args [ 2 ]:GetSteamId() ), player = args [ 2 ] }, false ) then
							Chat:Broadcast( "[Сервер] ", Color.White, args [ 2 ]:GetName() .." научился разговаривать благодаря ".. player:GetName(), Color ( 0, 255, 0 ) )
...
--							end
						...
						if mute:setPlayerMuted ( muteArgs, true ) then
							Chat:Broadcast( "[Сервер] ", Color.White, args [ 2 ]:GetName() .." получил бан чата от ".. player:GetName() .." на ".. tostring ( muteArgs.duration / 60 ) .." минут ( ".. tostring ( muteArgs.reason ) .." )", Color ( 255, 0, 0 ) )
--							for _, thePlayer in pairs ( self.canChat ) do
...
function Player:Message( msg, color )
	self:SendChatMessage( "[Сервер] ", Color.White, msg, msgColors [ color ] )
end
